...
- **`xlsx`** (SheetJS) is a `devDependency` used solely by `scripts/convert-ats34.mjs` at build
  time to convert the committed, SHA-256-pinned ATS34 workbook into the JSON fixture. It never
  reaches the browser bundle (`npm audit --omit=dev` is clean) and only ever processes one
  trusted, hash-verified input file — the prototype-pollution / ReDoS advisories require a
  malicious workbook, which cannot occur here. No upstream fix is available; no user-facing
  exposure exists.
