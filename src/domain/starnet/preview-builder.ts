...
export function buildSnprojPreview(
  ...
  const lines = [
    ...
    nativeProjectLine('create_dump_file', 1),
    nativeProjectLine('dump_rel_connections', 0),
    ...
    // Fallback for rows without explicit weights. Explicit DM weights already contain the
    // selected mm/ppm combination and centering contribution and are used verbatim by STAR*NET.
    nativeProjectLine('edm_ppm', w.distancePpm.toFixed(10)),
    nativeProjectLine('angle_std_err', nativeAngularSigma(w.angleArcSec, adjustment.angleOutputUnits).toFixed(10)),
    nativeProjectLine('direction_std_err', nativeAngularSigma(w.directionArcSec, adjustment.angleOutputUnits).toFixed(10)),
    nativeProjectLine('azimuth_std_err', nativeAngularSigma(w.azimuthArcSec, adjustment.angleOutputUnits).toFixed(10)),
    nativeProjectLine('zenith_std_err', nativeAngularSigma(w.zenithArcSec, adjustment.angleOutputUnits).toFixed(10)),
