...
function TargetsStep({ draft, update, onError }: { draft: WizardDraft; update: (p: Partial<WizardDraft>) => void; onError: (m: string) => void }) {
  ...
  const patchTarget = (index: number, patch: Partial<WizardDraft['targets'][number]>) => {
    const targets = draft.targets.map((target, targetIndex) => (targetIndex === index ? { ...target, ...patch } : target));
    update({ targets });
  };
  const resetPage = () => setPage(0);
  const stations = useMemo(
    () => [...new Set(draft.targets.map((target) => target.stationCode))].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })),
    [draft.targets],
  );
  const rows = useMemo(
